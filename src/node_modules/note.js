...
var Note = {
	...
	addNoteHistory: function(noteId, content) {
		...
		db.noteHistories.findOne({_id: noteId}, function(err, history) {
			...
			var newHistory = {Content: content, UpdatedTime: new Date()};

			// 新建之
			if(!history) {
				db.noteHistories.insert({_id: noteId, Histories: [newHistory], "UpdatedTime": new Date()});
			}
			// 更新之
			else {
		...
		db.noteHistories.findOne({_id: noteId}, function(err, doc) {
			if(err || !doc) {
				...
				for(var i = doc.Histories.length - 1; i >= 0; --i) {
					var contentAndDate = doc.Histories[i];
					var history;
					if(typeof contentAndDate == 'object') { // 第二版
						var history = {Content: contentAndDate.Content, UpdatedTime: new Date(contentAndDate.UpdatedTime)};
					} else { // 第一版只有Content, 没有UpdatedTime
						var history = {Content: doc.Histories[i], UpdatedTime: doc.UpdatedTime || new Date()}
					}
